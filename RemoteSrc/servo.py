...
class Servo:
    ...
    def getServoPwmValue(self,channel):
        value = 0
        channel = str(channel)
        if channel=='0':
            value = self.PwmServo.getServoValue(8)
        elif channel=='1':
            value = self.PwmServo.getServoValue(9)
        elif channel=='2':
            self.PwmServo.setServoPulse(10,500+int((angle+error)/0.09))
        elif channel=='3':
            self.PwmServo.setServoPulse(11,500+int((angle+error)/0.09))
    ...
    def get8BitValue(self,channel, angle, error=10):
        channel = str(channel)
        value = -1
        if channel=='0':
            value = angle + error
            value = 2500-int(value/0.09)
            value = value*4096/20000
            value = int(value) & 0xFF
        elif channel=='1':
            value = angle + error
            value = 500+int(value/0.09)
            value = value*4096/20000
            value = int(value) & 0xFF
