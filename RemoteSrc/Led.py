...
class Led:
    ...
    def getLedColor(self,index):
        packedValue = 0
        for i in range(8):
            if index & 0x01 == 1:
                packedValue = self.strip.getPixelColor(i)
            index=index >> 1
        
        white = (packedValue & SECOND_MASK) >> 24  
        red = (packedValue & THIRD_MASK) >> 16  
        green = (packedValue & FOURTH_MASK) >> 8
        blue = (packedValue & FIRST_MASK)
